...
void FKnotTrackCreator::CreateKnotTracks()
{
	...
	for (TSharedPtr<FKnotNodeTrack> KnotTrack : KnotTracks)
	{
		...
		for (int i = 0; i < NumCreations; i++)
		{
			...
			FVector2f KnotPos = Creation->KnotPos;
...
void FKnotTrackCreator::ExpandKnotTracks()
{
	...
	while (PendingTracks.Num() > 0)
	{
		...
		if (RelatedBottom.IsSet())
		{
			...
			ExpandedBounds = ExpandedBounds.OffsetBy(FVector2f(0, RelatedDeltaY));
...
UK2Node_Knot* FKnotTrackCreator::CreateKnotNode(FKnotNodeCreation* Creation, const FVector2f& Position, UEdGraphPin* ParentPin)
{
	if (!Creation)
	{
		return nullptr;
...
bool FKnotTrackCreator::TryAlignTrackToEndPins(TSharedPtr<FKnotNodeTrack> Track, const TArray<UEdGraphNode*>& AllNodes)
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		const FVector2f SourcePinPos = FBAUtils::GetPinPos(GraphHandler, SourcePin);
		const FVector2f OtherPinPos = FBAUtils::GetPinPos(GraphHandler, OtherPin);

		const FVector2f Padding = FBAUtils::IsParameterPin(OtherPin)
			? PinPadding
			: NodePadding;

		const FVector2f Point
			= SourcePin->Direction == EGPD_Output
			? FVector2f(OtherPinPos.X - Padding.X, SourcePinPos.Y)
			: FVector2f(OtherPinPos.X + Padding.X, SourcePinPos.Y);
...
bool FKnotTrackCreator::AnyCollisionBetweenPins(UEdGraphPin* Pin, UEdGraphPin* OtherPin)
{
	...
	const FVector2f PinPos = FBAUtils::GetPinPos(GraphHandler, Pin);
	const FVector2f OtherPinPos = FBAUtils::GetPinPos(GraphHandler, OtherPin);
...
bool FKnotTrackCreator::NodeCollisionBetweenLocation(FVector2f Start, FVector2f End, TSet<UEdGraphNode*> IgnoredNodes)
{
	TSet<UEdGraphNode*> FormattedNodes = Formatter->GetFormattedNodes();
...
void FKnotTrackCreator::AddNomadKnotsIntoComments()
{
	...
	for (UEdGraphNode* CreatedKnotNode : GetCreatedKnotNodes())
	{
		for (UEdGraphNode_Comment* Comment : CommentHandler->GetComments())
		{
			...
			if (CommentBounds.ContainsPoint(FVector2f(CreatedKnotNode->NodePosX, CreatedKnotNode->NodePosY)))
			{
...
{
	FVector2f ParentPinPos = FBAUtils::GetPinPos(GraphHandler, ParentPin);
	...
	{
		const FVector2f LinkedPinPos = FBAUtils::GetPinPos(GraphHandler, LinkedPin);
	...
	for (UEdGraphPin* OtherPin : LoopingPins)
	{
		...
		const FVector2f FirstKnotPos(ParentPinPos.X + 20, KnotTrack->GetTrackHeight());
		...
		const FVector2f SecondKnotPos(OtherPinPos.X - 20, KnotTrack->GetTrackHeight());
	...
	{
		const FVector2f PinPos = FBAUtils::GetPinPos(GraphHandlerRef, Pin);
	...
	const FVector2f LastPinPos = FBAUtils::GetPinPos(GraphHandler, LinkedPins.Last());
	...
	const FVector2f ToLast = LastPinPos - ParentPinPos;
	...
	const bool bNeedsReroute = !bSkipSameHeight || bPinNeedsTrack || bPreviousHasTrack;
	if (!bNeedsReroute)
	{
		return nullptr;
	}
	...
	const FVector2f MyKnotPos = FVector2f(ParentPinPos.X - NodePadding.X, KnotTrack->GetTrackHeight());
	...
	for (FBANodePinHandle& OtherPinHandle : KnotTrack->LinkedTo)
	{
		...
		const FVector2f KnotPos(KnotX, KnotTrack->GetTrackHeight());
...
TSharedPtr<FKnotNodeTrack> FKnotTrackCreator::MakeKnotTracksForParameterPins(UEdGraphPin* ParentPin, TArray<UEdGraphPin*> LinkedPins, TArray<TSharedPtr<FKnotNodeTrack>>& PreviousTracks)
{
	...
	const bool bLastPinFarAway = DistX > GetDefault<UBASettings>()->KnotNodeDistanceThreshold && !GetDefault<UBASettings>()->bExpandNodesByHeight;
	...
	const bool bNeedsReroute = bHeightDifference || bPinNeedsTrack || bPreviousHasTrack || bLastPinFarAway;
	if (!bNeedsReroute)
	{
		return nullptr;
	}
	...
	const FVector2f InitialKnotPos = FVector2f(ParentPinPos.X + PinPadding.X, KnotTrack->GetTrackHeight());
	...
	for (FBANodePinHandle& OtherPinHandle : KnotTrack->LinkedTo)
	{
		...
		const FVector2f KnotPos = FVector2f(KnotX, KnotTrack->GetTrackHeight());
...
void FKnotTrackCreator::AddKnotNodesToComments()
{
	...
	for (TSharedPtr<FKnotNodeTrack> Track : KnotTracks)
	{
		...
		for (const auto& Comment : CommentHandler->GetComments())
		{
			...
			bool bContainsSingleKnot = NumKnots == 1 && CommentBounds.ContainsPoint(FVector2f(SingleKnot->NodePosX, SingleKnot->NodePosY));
