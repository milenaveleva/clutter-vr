...
UK2Node_Knot* FKnotNodeCreation::CreateKnotNode(const FVector2f InKnotPos, UEdGraphPin* PreviousPin, UK2Node_Knot* KnotNodeToReuse, UEdGraph* Graph)
{
...
FSlateRect FKnotNodeTrack::GetTrackBounds()
{
	...
	return FSlateRect(FVector2f(TrackXLeft, LocalTrackY - (TrackSpacing - 1) * 0.5f),
					FVector2f(TrackXRight, LocalTrackY + (TrackSpacing - 1) * 0.5f));
}
...
void FKnotNodeTrack::SetTrackHeight(TSharedPtr<FFormatterInterface> Formatter)
{
	...
	for (int i = 0; i < 100; ++i)
	{
		...
		FVector2f StartPoint(TrackStart, TestSolution);
		FVector2f EndPoint(TrackEnd, TestSolution);
...
bool FKnotNodeTrack::TryAlignTrack(TSharedPtr<FFormatterInterface> Formatter, float TrackStart, float TrackEnd, float TestHeight)
{
	...
	for (UEdGraphNode* NodeToCollisionCheck : AllNodes)
	{
		FSlateRect NodeBounds = FBAUtils::GetCachedNodeBounds(GraphHandler, NodeToCollisionCheck).ExtendBy(FVector2f(0, TrackSpacing - 1));

		FVector2f StartPoint(TrackStart, TestHeight);
		FVector2f EndPoint(TrackEnd, TestHeight);
