...
void FEdGraphFormatter::FormatNode(UEdGraphNode* InitialNode)
{
	...
	const FVector2f SavedLocation = FVector2f(NodeToKeepStill->NodePosX, NodeToKeepStill->NodePosY);
...
void FEdGraphFormatter::ExpandByHeight()
{
	...
	for (UEdGraphNode* Node : NodePool)
	{
		...
		{
			const FVector2f ToPos = FBAUtils::GetPinPos(GraphHandler, Link.To);
			const FVector2f FromPos = FBAUtils::GetPinPos(GraphHandler, Link.From);
	...
	auto LinkedToSorter = [&GraphHandlerCapture, &NodesToCollisionCheck](UEdGraphPin& PinA, UEdGraphPin& PinB)
	{
		struct FLocal
		{
			...
			static UEdGraphPin* HighestPin(TSharedPtr<FBAGraphHandler> GraphHandler, UEdGraphPin* Pin, TSet<UEdGraphNode*>& VisitedNodes, bool& bHasEventNode, int32& DepthToEventNode)
			{
				...
				{
					const FVector2f PinPosA = FBAUtils::GetPinPos(GraphHandler, &PinA);
					const FVector2f PinPosB = FBAUtils::GetPinPos(GraphHandler, &PinB);
		...
		const FVector2f PinPosA = FBAUtils::GetPinPos(GraphHandlerCapture, HighestPinA);
		const FVector2f PinPosB = FBAUtils::GetPinPos(GraphHandlerCapture, HighestPinB);
...
bool FEdGraphFormatter::LinkToSort(UEdGraphPin& PinA, UEdGraphPin& PinB, TSet<UEdGraphNode*>& VisitedNodes)
{
	...
	const FVector2f PinPosA = FBAUtils::GetPinPos(GraphHandler, HighestPinA);
	const FVector2f PinPosB = FBAUtils::GetPinPos(GraphHandler, HighestPinB);
		...
		if (UEdGraphNode_Comment* CommentA = Cast<UEdGraphNode_Comment>(NodeA))
		{
			...
			for (UEdGraphNode* Node : CommentAContains)
			{
				...
				for (FPinLink PinLink : FBAUtils::GetPinLinks(Node, EGPD_Output))
				{
					...
					{
						FVector2f ToPos = FBAUtils::GetPinPos(GraphHandler, PinLink.To);
						FVector2f FromPos = FBAUtils::GetPinPos(GraphHandler, PinLink.From);
...
void FEdGraphFormatter::ResetRelativeToNodeToKeepStill(const FVector2f& SavedLocation)
{
...
void FEdGraphFormatter::PostFormatting()
{
	if (NodeToKeepStill)
	{
		PreviousNodeToKeepStillPosition = FVector2f(NodeToKeepStill->NodePosX, NodeToKeepStill->NodePosY);
	}
...
void FEdGraphFormatter::CenterBranches(UEdGraphNode* CurrentNode, TArray<ChildBranch>& ChildBranches, TSet<UEdGraphNode*>& NodesToCollisionCheck)
{
	...
	for (auto Node : NodesToCollisionCheck)
	{
		...
		if (FSlateRect::DoRectanglesIntersect(Bounds, AllNodesBounds))
		{
			...
			AllNodesBounds = AllNodesBounds.OffsetBy(FVector2f(0, OffsetY));
		}
	}
...
bool FEdGraphFormatter::NodeCollisionBetweenLocation(FVector2f Start, FVector2f End, TSet<UEdGraphNode*> IgnoredNodes)
{
	TSet<UEdGraphNode*> FormattedNodes = GetFormattedGraphNodes();
