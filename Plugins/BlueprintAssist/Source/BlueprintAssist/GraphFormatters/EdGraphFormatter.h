...
private:
	FVector2f PinPadding;
	FVector2f NodePadding;
	float TrackSpacing;
	...
	FVector2f PreviousNodeToKeepStillPosition;
	int32 LastFormattedX;
	int32 LastFormattedY;

	TMap<UEdGraphNode*, FVector2f> LastNodePositions;
	...
	bool NodeCollisionBetweenLocation(FVector2f Start, FVector2f End, TSet<UEdGraphNode*> IgnoredNodes);

	void FormatParameterNodes();

	void ResetRelativeToNodeToKeepStill(const FVector2f& SavedLocation);
