...
int32 SBlueprintAssistGraphOverlay::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	{
		const FVector2f Start = FBAUtils::GraphCoordToPanelCoord(GraphPanel, ToDraw.Start);
		const FVector2f End = FBAUtils::GraphCoordToPanelCoord(GraphPanel, ToDraw.End);
		TArray<FVector2f> LinePoints = { Start, End };
	...
	for (const FBAGraphOverlayBounds& ToDraw : BoundsToDraw)
	{
		...
		{
			const FVector2f TL = FBAUtils::GraphCoordToPanelCoord(GraphPanel, ToDraw.Bounds.GetTopLeft());
			const FVector2f TR = FBAUtils::GraphCoordToPanelCoord(GraphPanel, ToDraw.Bounds.GetTopRight());
			const FVector2f BL = FBAUtils::GraphCoordToPanelCoord(GraphPanel, ToDraw.Bounds.GetBottomLeft());
			const FVector2f BR = FBAUtils::GraphCoordToPanelCoord(GraphPanel, ToDraw.Bounds.GetBottomRight());
			TArray<FVector2f> LinePoints = { TL, TR, BR, BL, TL };
	...
	for (auto Node : CurrentGraphHandler->GetFocusedEdGraph()->Nodes)
	{
		if (CurrentGraphHandler->GetNodeData(Node).bLocked)
		{
			...
			DrawIconOnNode(OutDrawElements, OutgoingLayer, GraphNode, GraphPanel, CachedLockBrush, ImageSize, FVector2f(0, 1.0f));
		}
	}
...
{
	const FVector2f TL = FBAUtils::GraphCoordToPanelCoord(GraphPanel, Bounds.GetTopLeft());
	const FVector2f TR = FBAUtils::GraphCoordToPanelCoord(GraphPanel, Bounds.GetTopRight());
	const FVector2f BL = FBAUtils::GraphCoordToPanelCoord(GraphPanel, Bounds.GetBottomLeft());
	const FVector2f BR = FBAUtils::GraphCoordToPanelCoord(GraphPanel, Bounds.GetBottomRight());
	...
	const FVector2f& IconOffset) const
{
	...
	{
		FVector2f Offset = IconSize * -0.5f + NodeBounds.GetSize() * IconOffset;
