...
struct BLUEPRINTASSIST_API FBAUtils
{
	...
	static FVector2f GetNodePosition(UEdGraphNode* Node);
	...
	static FVector2f GetPinPos(TSharedPtr<FBAGraphHandler> GraphHandler, UEdGraphPin* Pin);
	static FVector2f GetPinPos(TSharedPtr<SGraphPin> Pin);
	...
	static void LinkKnotNodeBetween(UK2Node_Knot* Node, const FVector2f& Position, UEdGraphPin* PinA, UEdGraphPin* PinB);
	...
	static FSlateRect FSlateRectFromVectors(const FVector2f& A, const FVector2f& B);
	...
	static FVector2f SnapToGrid(const FVector2f& Position);
	...
	static FVector2f GraphCoordToPanelCoord(
		TSharedPtr<SGraphPanel> GraphPanel,
		const FVector2f& PanelSpaceCoordinate);

	static FVector2f ScreenSpaceToPanelCoord(TSharedPtr<SGraphPanel> GraphPanel, const FVector2f& ScreenSpace);
	...
	static void MoveComment(UEdGraphNode_Comment* Comment, FVector2f Delta);
	...
	static TSharedPtr<IMenu> OpenPopupMenu(
		...
		const FVector2f& PopupDesiredSize,
		const FVector2f& WindowOffset = FVector2f::ZeroVector,
		const FVector2f& MenuAnchor = FVector2f(0.5f, 0.5f));
