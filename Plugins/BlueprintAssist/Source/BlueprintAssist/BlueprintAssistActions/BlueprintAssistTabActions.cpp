...
void FBATabActions::ShiftCameraInDirection(int X, int Y) const
{
	if (TSharedPtr<FBAGraphHandler> GraphHandler = GetGraphHandler())
	{
		...
		float Zoom;
		FVector2f CurrentViewLocation;
		GraphHandler->GetGraphEditor()->GetViewLocation(CurrentViewLocation, Zoom);

		/** Shift the current view location */
		const FVector2f Offset = FVector2f(X, Y) * GetDefault<UBASettings>()->ShiftCameraDistance / Zoom;

		const FVector2f CurrentLocation = GraphHandler->IsLerpingViewport() ? GraphHandler->GetTargetLerpLocation() : CurrentViewLocation;
