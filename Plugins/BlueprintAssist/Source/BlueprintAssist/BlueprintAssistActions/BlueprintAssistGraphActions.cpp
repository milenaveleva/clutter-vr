...
#include "BlueprintAssist/BlueprintAssistWidgets/BABlueprintActionMenu.h"
...
void FBAGraphActions::OpenContextMenu(const FVector2f& MenuLocation, const FVector2f& NodeSpawnPosition)
{
...
void FBAGraphActions::OpenContextMenuFromPin(UEdGraphPin* Pin, const FVector2f& MenuLocation, const FVector2f& NodeLocation)
{
	TSharedPtr<FBAGraphHandler> GraphHandler = GetGraphHandler();
	if (!GraphHandler)
	{
...
void FBAGraphActions::OnOpenContextMenu()
{
	...
	const FVector2f MenuLocation = FSlateApplication::Get().GetCursorPos();
