...
bool FBAInputProcessor::HandleMouseMoveEvent(FSlateApplication& SlateApp, const FPointerEvent& MouseEvent)
{
	...
	if (TSharedPtr<SGraphPanel> GraphPanel = GraphHandler->GetGraphPanel())
	{
		const FVector2f NewMousePos = FBAUtils::SnapToGrid(FBAUtils::ScreenSpaceToPanelCoord(GraphPanel, MouseEvent.GetScreenSpacePosition()));
		const FVector2f Delta = NewMousePos - LastMousePos;
...
bool FBAInputProcessor::BeginGroupMovement(const FKey& Key)
{
	...
	if (Key == EKeys::LeftMouseButton)
	{
		...
		if (!HoveredPin && !FBAUtils::ContainsWidgetInFront(HoveredNode, BlockingWidgets))
		{
			...
			LastAnchorPos = HoveredNode->GetPosition2f();
...
bool FBAInputProcessor::OnMouseDrag(FSlateApplication& SlateApp, const FVector2f& MousePos, const FVector2f& Delta)
{
...
void FBAInputProcessor::UpdateGroupMovement()
{
	...
	const FVector2f NewNodePos(AnchorNode->NodePosX, AnchorNode->NodePosY);
	const FVector2f Delta = NewNodePos - LastAnchorPos;
...
void FBAInputProcessor::GroupMoveSelectedNodes(const FVector2f& Delta)
{
	TSharedPtr<FBAGraphHandler> GraphHandler = FBATabHandler::Get().GetActiveGraphHandler();
...
void FBAInputProcessor::GroupMoveNodes(const FVector2f& Delta, TSet<UEdGraphNode*>& Nodes)
{
	TSharedPtr<FBAGraphHandler> GraphHandler = FBATabHandler::Get().GetActiveGraphHandler();
	TSet<UEdGraphNode*> SelectedNodes = GraphHandler->GetSelectedNodes();
