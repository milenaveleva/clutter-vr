...
FVector2f FBAUtils::GetNodePosition(UEdGraphNode* Node)
{
	return FVector2f(Node->NodePosX, Node->NodePosY);
}
...
FSlateRect FBAUtils::GetNodeBounds(TSharedPtr<SGraphNode> GraphNode)
{
	if (GraphNode)
	{
		return FSlateRect::FromPointAndExtent(GraphNode->GetPosition2f(), GraphNode->GetDesiredSize());
	}
...
FSlateRect FBAUtils::GetPinBounds(TSharedPtr<SGraphPin> GraphPin)
{
	...
	const FVector2f Pos = FBAUtils::GetPinPos(GraphPin);
	const FVector2f Size = GraphPin->GetDesiredSize();
...
FVector2f FBAUtils::GetPinPos(TSharedPtr<FBAGraphHandler> GraphHandler, UEdGraphPin* Pin)
{
...
FVector2f FBAUtils::GetPinPos(TSharedPtr<SGraphPin> Pin)
{
	UEdGraphNode* OwningNode = Pin->GetPinObj()->GetOwningNode();
	if (OwningNode)
	{
		return FVector2f(OwningNode->NodePosX, OwningNode->NodePosY) + Pin->GetNodeOffset();
	}
...
void FBAUtils::LinkKnotNodeBetween(UK2Node_Knot* KnotNode, const FVector2f& Position, UEdGraphPin* PinA, UEdGraphPin* PinB)
{
#if BA_UE_VERSION_OR_LATER(5, 1)
	const static FVector2f NodeSpacerSize(42.f, 20.0f);
#else
	const static FVector2f NodeSpacerSize(42.f, 16.0f);
#endif


	// This constant is duplicated from inside of SGraphNodeKnot
	const FVector2f KnotTopLeft = Position - NodeSpacerSize * 0.5f;
...
FSlateRect FBAUtils::FSlateRectFromVectors(const FVector2f& A, const FVector2f& B)
{
	return FSlateRect(
		FMath::Min(A.X, B.X),
		FMath::Min(A.Y, B.Y),
	...
	const FVector2f& End)
{
	...
	const FVector2D LocalStart(Start);
	const FVector2D LocalEnd(End);

	const FVector Start3D(LocalStart, 0.f);
	const FVector End3D(LocalEnd, 0.f);
...
bool FBAUtils::ArePinsStraightened(TSharedPtr<FBAGraphHandler> GraphHandler, FPinLink& PinLink)
{
	...
	const FVector2f FromPos = GetPinPos(GraphHandler, FromPin);
	const FVector2f ToPos = GetPinPos(GraphHandler, ToPin);
...
void FBAUtils::GetChildWidgets(TSharedPtr<SWidget> Widget,
	const TArray<FName>& WidgetClassNames, TSet<TSharedPtr<SWidget>>& OutWidgets)
{
	if (Widget.IsValid())
	{
		...
		if (FChildren* Children = Widget->GetChildren())
		{
			for (int i = 0; i < Children->Num(); i++)
			{
				GetChildWidgets(Children->GetChildAt(i), WidgetClassNames, OutWidgets);
			}
		}
	}
}
...
TSharedPtr<SGraphNode> FBAUtils::GetHoveredGraphNode(TSharedPtr<SGraphPanel> GraphPanel)
{
	...
	const FVector2f CursorInPanel = FBAUtils::ScreenSpaceToPanelCoord(GraphPanel, FSlateApplication::Get().GetCursorPos());
...
FVector2f FBAUtils::SnapToGrid(const FVector2f& Position)
{
...
FVector2f FBAUtils::GraphCoordToPanelCoord(
	TSharedPtr<SGraphPanel> GraphPanel,
	const FVector2f& GraphSpaceCoordinate)
{
	return (GraphSpaceCoordinate - GraphPanel->GetViewOffset()) * GraphPanel->GetZoomAmount();
}

FVector2f FBAUtils::ScreenSpaceToPanelCoord(TSharedPtr<SGraphPanel> GraphPanel, const FVector2f& ScreenSpace)
{
...
void FBAUtils::InteractWithWidget(TSharedPtr<SWidget> Widget)
{
	...
	if (FBAUtils::IsClickableWidget(Widget))
	{
		...
		const FVector2f GeometryCenter = CachedGeometry.GetAbsolutePosition() + CachedGeometry.
...
bool FBAUtils::IsExtraRootNode(UEdGraphNode* Node)
{
	...
	FMetaData MetaData = GetNodeMetaData(Node);
...
void FBAUtils::MoveComment(UEdGraphNode_Comment* Comment, FVector2f Delta)
{
...
TSharedPtr<IMenu> FBAUtils::OpenPopupMenu(TSharedRef<SWidget> WindowContents, const FVector2f& PopupDesiredSize, const FVector2f& Offset, const FVector2f& MenuAnchor)
{
	...
	if (ParentWindow.IsValid())
	{
		...
		const FVector2f MonitorCenter((ParentMonitorRect.Right + ParentMonitorRect.Left) * (0.5f + Offset.X), (ParentMonitorRect.Top + ParentMonitorRect.Bottom) * (0.5f + Offset.Y));
		const FVector2f PopupAnchor = PopupDesiredSize * MenuAnchor;
		const FVector2f WindowPosition = MonitorCenter - PopupAnchor;
...
{
	checkf(Node, TEXT("GetNodeMetaData: Node was null"));
	UPackage* Package = Node->GetOutermost();
	checkf(Package, TEXT("GetNodeMetaData: Node '%s' had no outermost package"), *GetNameSafe(Node));
