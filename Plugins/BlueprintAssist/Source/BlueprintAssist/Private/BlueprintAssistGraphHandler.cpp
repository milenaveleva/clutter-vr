				...
				if (FBAUtils::IsNodeImpure(OtherLinkedNode))
				{
					...
					if (ExecPins.FilterByPredicate(FBAUtils::IsPinLinked).Num() == 0)
					{
						...
						if (OtherExecPin->LinkedTo.Num() > 0)
						{
							...
							{
								FBAUtils::TryCreateConnection(OtherExecPin->LinkedTo[0], MyPinsInDirection[0], false);
							}
						}

						FBAUtils::TryCreateConnection(ExecPins[0], OtherExecPin, false);
						return;
					}
				}
			}
...
void FBAGraphHandler::AutoZoomToNode(UEdGraphNode* Node)
{
	...
	else
	{
		...
		const FVector2f NodePos(Node->NodePosX, Node->NodePosY);
		BeginLerpViewport(NodePos);
	}
}
...
void FBAGraphHandler::UpdateCachedNodeSize(float DeltaTime)
{
	...
	if (PendingSize.Num() > 0)
	{
		...
		if (FocusedNode != FirstNode)
		{
			...
			GraphEditor->SetViewLocation(FVector2f(FocusedNode->NodePosX, FocusedNode->NodePosY), 1.f);
		}
		else
		{
			GraphEditor->SetViewLocation(FVector2f(FocusedNode->NodePosX, FocusedNode->NodePosY), 1.f);
	...
	for (UEdGraphNode* Node : PendingSize)
	{
		...
		if (FBAUtils::IsCommentNode(Node))
		{
			Size.Y = GraphNode->GetDesiredSizeForMarquee2f().Y;
		}
...
void FBAGraphHandler::UpdateLerpViewport(const float DeltaTime)
{
	if (bLerpViewport)
	{
		...
		FVector2f TargetView = TargetLerpLocation;
		if (bCenterWhileLerping)
		{
			...
			const FVector2f HalfOfScreenInGraphSpace = 0.5f * LocalSize / GraphPanel->GetZoomAmount();
			TargetView -= HalfOfScreenInGraphSpace;
		}

		if (FVector2f::Distance(CurrentView, TargetView) > 10.f)
		{
			const FVector2f NewView = FMath::Vector2DInterpTo(CurrentView, TargetView, DeltaTime, 10.f);
...
void FBAGraphHandler::BeginLerpViewport(const FVector2f TargetView, const bool bCenter)
{
