...
FConnectionDrawingPolicy* FENConnectionDrawingPolicyFactory::CreateConnectionPolicy(const class UEdGraphSchema* Schema, int32 InBackLayerID, int32 InFrontLayerID, float ZoomFactor, const class FSlateRect& InClippingRect, class FSlateWindowElementList& InDrawElements, class UEdGraph* InGraphObj) const
{
    ...
    */
    ...
    if (ElectronicNodesSettings.ActivateOnVoxelPlugin)
    {
        if (Schema->GetClass()->GetFName() == "VoxelGraphSchema")
        {
            return new FENConnectionDrawingPolicy(InBackLayerID, InFrontLayerID, ZoomFactor, InClippingRect, InDrawElements, InGraphObj);
        }
    }
...
void FENConnectionDrawingPolicy::DrawConnection(int32 LayerId, const FVector2f& Start, const FVector2f& End, const FConnectionParams& Params)
{
    ...
    bool RightPriority = ENIsRightPriority(Params);
    ...
    FVector2f StartDirection = (Params.StartDirection == EEdGraphPinDirection::EGPD_Output) ? FVector2f(1.0f, 0.0f) : FVector2f(-1.0f, 0.0f);
    FVector2f EndDirection = (Params.EndDirection == EEdGraphPinDirection::EGPD_Input) ? FVector2f(1.0f, 0.0f) : FVector2f(-1.0f, 0.0f);

    FVector2f new_Start = Start;
    FVector2f new_End = End;

    ENCorrectZoomDisplacement(new_Start, new_End);
    ENProcessRibbon(_LayerId, new_Start, StartDirection, new_End, EndDirection, Params);
    ...
    if (Settings->bTreatSplinesLikePins)
    {
        ...
        if (bCloseToSpline)
        {
            if (ClosestDistanceSquared < SplineOverlapResult.GetDistanceSquared())
            {
                ...
                SplineOverlapResult = FGraphSplineOverlapResult(Params.AssociatedPin1, Params.AssociatedPin2, ClosestDistanceSquared, SquaredDistToPin1, SquaredDistToPin2, bCloseToSpline);
            }
        }
    }
}

void FENConnectionDrawingPolicy::ENCorrectZoomDisplacement(FVector2f& Start, FVector2f& End)
{
...
FVector2f FENConnectionDrawingPolicy::GetConvertedMousePosition() const
{
    return FVector2f(LocalMousePosition);
}
...
{
    const FVector2f TemporaryPoint = FMath::ClosestPointOnSegment2D(GetConvertedMousePosition(), Start, End);
...
void FENConnectionDrawingPolicy::ENComputeClosestPointDefault(const FVector2f& Start, const FVector2f& StartTangent, const FVector2f& End, const FVector2f& EndTangent)
{
    ...
    if (bCloseToSpline)
    {
        ...
        for (float TestAlpha = 0.0f; TestAlpha < 1.0f; TestAlpha += StepInterval)
        {
            ...
            const FVector2f ClosestPointToSegment = FMath::ClosestPointOnSegment2D(GetConvertedMousePosition(), PointOnSpline1, PointOnSpline2);
