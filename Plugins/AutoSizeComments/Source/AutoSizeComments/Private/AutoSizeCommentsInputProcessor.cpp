...
static TSharedPtr<FAutoSizeCommentsInputProcessor> CommentsInputProcessorInstance;

void FAutoSizeCommentsInputProcessor::Create()
{
	CommentsInputProcessorInstance = MakeShareable(new FAutoSizeCommentsInputProcessor());
	FSlateApplication::Get().RegisterInputPreProcessor(CommentsInputProcessorInstance);

	CommentsInputProcessorInstance->Init();
}

void FAutoSizeCommentsInputProcessor::Cleanup()
{
	if (CommentsInputProcessorInstance.IsValid())
	{
		if (FSlateApplication::IsInitialized())
		{
			FSlateApplication::Get().UnregisterInputPreProcessor(CommentsInputProcessorInstance);
		}

		CommentsInputProcessorInstance.Reset();
	}
}
...
{
	return *CommentsInputProcessorInstance;
}
