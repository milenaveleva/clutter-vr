...
void SAutoSizeCommentsGraphNode::MoveTo(const FVector2f& NewPosition, FNodeSet& NodeFilter, bool bMarkDirty)
#else
...
#endif
{
	...
	FVector2f NewPos = GetPosition2f() + PositionDelta;
...
FReply SAutoSizeCommentsGraphNode::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	{
		const FVector2f MousePositionInNode = MyGeometry.AbsoluteToLocal(MouseEvent.GetScreenSpacePosition());
...
FReply SAutoSizeCommentsGraphNode::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (bUserIsDragging)
	{
		FVector2f Padding(5, 5);
		FVector2f MousePositionInNode = MyGeometry.AbsoluteToLocal(MouseEvent.GetScreenSpacePosition());
...
{
	FVector2D LocalUserSize(UserSize);
	return LocalUserSize;
}
...
bool SAutoSizeCommentsGraphNode::CanBeSelected(const FVector2f& MousePositionInNode) const
{
...
FVector2f SAutoSizeCommentsGraphNode::GetDesiredSizeForMarquee2f() const
{
	return FVector2f(UserSize.X, GetTitleBarHeight());
}
...
FCursorReply SAutoSizeCommentsGraphNode::OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	...
	const FVector2f LocalMouseCoordinates = MyGeometry.AbsoluteToLocal(CursorEvent.GetScreenSpacePosition());
...
int32 SAutoSizeCommentsGraphNode::GetSortDepth() const
{
	...
	const FVector2f LocalPos = GetCachedGeometry().AbsoluteToLocal(FSlateApplication::Get().GetCursorPos());
...
FSlateRect SAutoSizeCommentsGraphNode::GetTitleRect() const
{
	...
	const FVector2f NodeSize = TitleBar.IsValid() ? TitleBar->GetDesiredSize() : GetDesiredSize();
...
void SAutoSizeCommentsGraphNode::UpdateRefreshDelay()
{
	...
	const FVector2f NodePosition = GetPosition2f();
	const FVector2f BottomRight = NodePosition + FVector2f(1, 1);
...
void SAutoSizeCommentsGraphNode::ResizeToFit()
{
	...
	if (CommentNode->GetNodesUnderComment().Num() > 0)
	{
		...
		FVector2f CurrSize = Bounds.GetSize();
		...
		FVector2f DesiredPos = Bounds.GetTopLeft();
		DesiredPos.Y -= TitleBarHeight;

		// move to desired pos
		if (!GetPosition2f().Equals(DesiredPos, .1f))
		{
...
void SAutoSizeCommentsGraphNode::SnapVectorToGrid(FVector2f& Vector)
{
...
void SAutoSizeCommentsGraphNode::QueryNodesUnderComment(TArray<TSharedPtr<SGraphNode>>& OutNodesUnderComment, const ECommentCollisionMethod OverrideCollisionMethod, const bool bIgnoreKnots)
{
	...
	// Get our geometry
	FVector2f NodePosition = GetPosition2f();
...
void SAutoSizeCommentsGraphNode::AdjustMinSize(FVector2f& InSize)
{
	const float MinY = IsHeaderComment() ? 0 : 80;
	InSize = FVector2f(FMath::Max(125.f, InSize.X), FMath::Max(GetTitleBarHeight() + MinY, InSize.Y));
}
	...
	if (LocalGraphNode.IsValid())
	{
		...
		if (UAutoSizeCommentsSettings::Get().bUseCommentBubbleBounds && Node->bCommentBubbleVisible)
		{
			if (FNodeSlot* CommentSlot = LocalGraphNode->GetSlot(ENodeZone::TopCenter))
			{
				...
				{
					FVector2f CommentBubbleSize = LocalCommentBubble->GetSize2f();
