    ...
    const TScriptInterface<IIsdkIHandPointerPose>& InHandPointerPoseDataSource)
{
  ...
  if (IsValid(ParentMotionController))
  {
    PointerPoseComponent->AttachToComponent(
        ParentMotionController, FAttachmentTransformRules(EAttachmentRule::SnapToTarget, false));
  }
}
...
void UIsdkControllerMeshComponent::UpdateController()
{
  if (IsValid(RootPoseDataSource.GetObject()))
  {
    ...
    if (bIsControllerPoseValid)
    {
      ...
      LastUpdatedRootPose = ControllerPose;
  ...
  if (IsValid(HandPointerPoseDataSource.GetObject()))
  {
    ...
    // If we're following a bone on a component, set it to that location
    if (SocketFollowComponent != nullptr)
    {
      if (!PoseOverride->IsRootOverrideActive())
      {
        PointerPose = SocketFollowComponent->GetSocketTransform(SocketFollowBoneName);
        PointerPoseComponent->SetWorldTransform(PointerPose);
      }
    }
    else
    {
      IIsdkIHandPointerPose::Execute_GetRelativePointerPose(
          HandPointerPoseDataSource.GetObject(), PointerPose, bIsPointerPoseValid);
      if (bIsPointerPoseValid)
      {
        PointerPoseComponent->SetRelativeTransform(PointerPose);
      }
    }
  }
}
