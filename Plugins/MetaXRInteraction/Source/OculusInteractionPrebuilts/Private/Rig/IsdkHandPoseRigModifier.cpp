    ...
    UIsdkRigComponent* CallingRigComponent)
{
  ...
  if (!IsValid(CallingRigComponent))
  {
    UE_LOG(
        ...
        Error,
        TEXT("UIsdkHandPoseRigModifier::InitializeRigModifier - CallingRigComponent is null!"));
    return;
  }
  ...
  if (HandPoseSubsystem == nullptr)
  {
    UE_LOG(
        LogOculusInteractionPrebuilts,
        Error,
        TEXT("UIsdkHandPoseRigModifier::InitializeRigModifier - HandPoseSubsystem is null!"));
    return;
  }

  if (CallingRigComponent->IsA(UIsdkControllerRigComponent::StaticClass()))
  {
    bControllerRig = true;
  }
  ...
  if (!IsValid(VisualComponent))
  {
    UE_LOG(
        ...
        Error,
        TEXT(
            "UIsdkHandPoseRigModifier::InitializeRigModifier - UIsdkTrackedDataSourceRigComponent is null!"));
    return;
  }
  ...
  if (!IsValid(HandMeshComponent))
  {
    UE_LOG(
        ...
        Error,
        TEXT(
            "UIsdkHandPoseRigModifier::InitializeRigModifier - HandMeshComponent cast to UIsdkHandMeshComponent is null!"));
    return;
  }
    ...
    const FIsdkInteractorStateEvent& StateEvent)
{
  ...
  if (bControllerRig)
  {
    // We only apply hand poses for controllers if we're currently in a controller mode that is
    // rendering a hands-only supported mesh
    const auto ControllerHandBehavior = UIsdkFunctionLibrary::GetControllerHandBehavior(GetWorld());
    if (ControllerHandBehavior != EControllerHandBehavior::HandsOnlyProcedural)
    {
      return;
    }
  }
